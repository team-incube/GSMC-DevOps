resource "aws_iam_role" "db_instance_profile" {
...
resource "aws_iam_role_policy_attachment" "attachment_s3-ReadOnly" {
  role = aws_iam_role.db_instance_profile.name
...
resource "aws_iam_role_policy_attachement" "db_instance_profile" {
  role = aws_iam_role.db_instance_profile.name
...
resource "aws_instance" "db_instance" {
  ami = data.aws_ami.ubuntu.id
    ...
    iam_instance_profile = aws_iam_role.db_instance_profile.name


    vpc_security_group_ids = [
      aws_security_group.db-db.id
    ]
    tags = {
